...
class Env(ABC):
    def __init__(self, config: Dict[str, Any], sim_device: str, graphics_device_id: int, headless: bool):
        ...
        self.act_space = spaces.Box(np.ones(self.num_actions, dtype=np.float32) * -1.0, np.ones(self.num_actions, dtype=np.float32) * 1.0)

        self.clip_obs = config["env"].get("clipObservations", np.Inf)
        self.clip_actions = config["env"].get("clipActions", np.Inf)

        self.control_freq_inv = config["env"]["controlFrequencyInv"]
...
class VecTask(Env):
    ...
    def _allocate_buffers(self):
        ...
        self.obs_buf_lag_history = torch.zeros((self.num_envs, 80, self.num_obs // self.obs_hist_len), device=self.device, dtype=torch.float)
